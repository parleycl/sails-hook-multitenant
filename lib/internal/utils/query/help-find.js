...
var getAttribute = require('waterline/lib/waterline/utils/ontology/get-attribute');
...
module.exports = function helpFind(WLModel, s2q, omen, done, req) {
  ...
  (function _getPopulatedRecords(proceed){
    ...
    var doJoinsInParentAdapter = (function () {
      ...
      _.all(parentQuery.joins, function(join) {
        ...
        if (collections[join.childCollectionIdentity].multitenant && !WLModel.multitenant) return false;
      ...
      parentAdapter.find(parentDatastoreName, parentQueryWithoutJoins, function (err, parentResults) {
        ...
        async.reduce(_.keys(joinsByAlias), parentResults, function(populatedParentRecords, alias, nextSetOfJoins) {
          ...
          if (aliasJoins.length === 2) {
            ...
            var ignore_tenancy = !junctionTableModel.multitenant;

            junctionTableModel.tenancy(req, ignore_tenancy, WLModel).then(model => {
              // Select the datastore from multitenancy
              junctionTableDatastoreName = model.datastore;
              ...
              junctionTableAdapter.find(junctionTableDatastoreName, junctionTableQuery, function(err, junctionTableResults) {
                ...
                var ignore_tenancy = !childTableModel.multitenant;
                childTableModel.tenancy(req, ignore_tenancy, WLModel).then(model => {
                  // Select the datastore from tenancy
                  childTableDatastoreName = model.datastore;
          ...
          // Otherwise, if there's one join in the set: no junction table.
            ...
            childTableModel.tenancy(req, ignore_tenancy, WLModel).then(model => {
              // Asign the datastore with tenancy selection
              childTableDatastoreName = model.datastore;
