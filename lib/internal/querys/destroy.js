      ...
      _self.tenancy(req, ignore_tenancy).then(model => {
        ...
        (function _runBeforeLC(proceed) {
          ...
          if (ignore_tenancy) {
            ...
            WLModel._callbacks.beforeDestroy(req, query.criteria, function (err){
              if (err) { return proceed(err); }
              return proceed(undefined, query);
            });
          }
                ...
                (function _maybeTransformRecords(proceed){
                  ...
                  async.each(transformedRecordsMaybe, function _eachRecord(record, next) {
                    ...
                    if (ignore_tenancy) {
                      ...
                      WLModel._callbacks.afterDestroy(req, record, function _afterMaybeRunningAfterDestroyForThisRecord(err) {
                        if (err) {
                          return next(err);
                        }
