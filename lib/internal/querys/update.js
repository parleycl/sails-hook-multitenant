      ...
      _self.tenancy(req, ignore_tenancy).then(model => {
        ...
        (function(proceed) {
          ...
          if (ignore_tenancy) {
            ...
            WLModel._callbacks.beforeUpdate(req, query.valuesToSet, function(err){
              if (err) { return proceed(err); }
              return proceed(undefined, query);
            });
          }
            ...
            function _afterReplacingAllCollections(err) {
              ...
              async.each(transformedRecords, function _eachRecord(record, next) {
                ...
                if (ignore_tenancy) {
                  ...
                  WLModel._callbacks.afterUpdate(req, record, function _afterMaybeRunningAfterUpdateForThisRecord(err) {
                    if (err) {
                      return next(err);
                    }
